...
@pytest.mark.asyncio
async def test_find_one(db: Database) -> None:
...
@pytest.mark.asyncio
async def test_distinct(db: Database) -> None:
...
@pytest.mark.asyncio
async def test_find_many_sort(db: Database) -> None:
...
@pytest.mark.asyncio
async def test_find_many_skip_take(db: Database) -> None:
...
@pytest.mark.asyncio
async def test_find_many_filter(db: Database) -> None:
...
@pytest.mark.asyncio
async def test_find_sort(db: Database) -> None:
...
@pytest.mark.asyncio
async def test_find_skip_take(db: Database) -> None:
...
@pytest.mark.asyncio
async def test_find_filter(db: Database) -> None:
...
@pytest.mark.asyncio
async def test_find_filter_by_regex(db: Database) -> None:
...
@pytest.mark.asyncio
async def test_aggregate_project(db: Database) -> None:
    collection = db["test_aggregate_project"]
    await collection.insert_one({"a": 1, "b": 2})
    cur = await collection.aggregate(pipeline=[{"$project": {"_id": False, "b": False}}])
    assert (await cur.to_list())[0] == {"a": 1}


@pytest.mark.asyncio
async def test_aggregate_group(db: Database) -> None:
    ...
    docs = [doc for doc in docs if doc["value"] > 1]  # type:ignore[operator]
    docs = sorted(docs, key=lambda doc: doc["name"])  # type:ignore[return-value,arg-type]
    docs = [
