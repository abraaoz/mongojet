import pytest
from mongojet import Collection, Database, IndexModel, IndexModelDef, OperationFailure


async def get_index_by_name(c: Collection, name: str) -> IndexModelDef:
...
@pytest.mark.asyncio
async def test_create_index(db: Database) -> None:
...
@pytest.mark.asyncio
async def test_create_index_with_options(db: Database) -> None:
    col_name = "test_create_index_with_options"
    collection = db[col_name]

    key = "key"
    unique = True
    expireAfterSeconds = 3600  # noqa: N806
    background = True
    sparse = True
...
@pytest.mark.asyncio
async def test_create_partial_index(db: Database) -> None:
    col_name = "test_create_partial_index"
    collection = db[col_name]

    key = {"key": 1}
    partialFilterExpression = {"field1": {"$gt": 0}}  # noqa: N806
...
@pytest.mark.asyncio
async def test_create_text_index(db: Database) -> None:
...
@pytest.mark.asyncio
async def test_create_2d_index(db: Database) -> None:
...
@pytest.mark.asyncio
async def test_create_2dsphere_index(db: Database) -> None:
...
@pytest.mark.asyncio
async def test_create_hashed_index(db: Database) -> None:
...
@pytest.mark.asyncio
async def test_create_indexes(db: Database) -> None:
...
@pytest.mark.asyncio
async def test_create_index_error(db: Database) -> None:
